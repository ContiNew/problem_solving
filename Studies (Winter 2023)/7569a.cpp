...
int main() {
		...
		for (int j = 0; j < n; j++) { // 세로에 대해서
			for (int i = 0; i < m; i++) { // 가로에 대해서 
				...
				if (tmp == RED) { // 익은 토마토일 경우 위치를 체크한다.
					vector<int> loc = { i,j,k }; //vector를 이용해 위치좌표를 잡아준다. 마지막 파라미터는 이 토마토가 익기위해 경과한시간.
	...
	while (!current_breadth.empty()) {
		...
		for (int i = 0; i < 6; i++) { // 가능한 모든 방향에 대해서 BFS 순회를 진행한다. 
			...
			tomatoes[z][y][x] = 1 + tomatoes[cur[2]][cur[1]][cur[0]]; // 이 경우에는 순회하면서 더 깊은 깊이에 있는 토마토에 가중치를 더해준다
			vector<int> loc = {x,y,z}; // 그 토마토의 위치에 대해서 저장하고
		...
		for (int j = 0; j < n; j++) { // 세로에 대해서
			for (int i = 0; i < m; i++) { // 가로에 대해서 
				if (tomatoes[k][j][i] == 0) {
					...
					ans = max(ans, tomatoes[k][j][i]); // 가장 마지막에 탐색한 토마토의 가중치 값이 구하고자 했던 일자이다
				}
			}
		}
	}
	cout << ans - 1 << "\n"; // 0일일때 붉은 토마토의 상태가 1이므로 빼준다
