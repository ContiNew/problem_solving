...
vector<int> graphT[1250]; // 2차로 DFS를 돌 전치그래프, 후에 간선을 제거해서 다시 돈다
...
int main() {
	...
	//		int dstVertex = graphT[j][i];
	//		if (scc_gids[j] == scc_gids[dstVertex]) { // 같은 scc 그룹 안에 있을 때
	//			graphT[j].erase(graphT[j].begin() + i);
	//			i--;// 전치그래프에서 간선을 삭제한다.
	//		}
	//	}
	//}
	// scc가 존재하지 않는 전치 그래프로 만든다. 


	for (int j = 0; j < vertices; j++) {
		for (vector<int>::iterator it = graphT[j].begin(); it != graphT[j].end(); ) {
			int dstVertex = *it;
			if (scc_gids[j] == scc_gids[dstVertex]) {
				it = graphT[j].erase(it);
			}
			else {
				it++;
			}
